...
main = defaultMain $ do
    testGroup
      ...
      , bad  "User 1 locks and user 2 takes with R = -42 before dealine fails  " $ testScript 5_000 (-42)
      , bad  "User 1 locks and user 2 takes with R = 0   before dealine fails  " $ testScript 5_000 0
      , bad  "User 1 locks and user 2 takes with R = 42  before dealine fails  " $ testScript 5_000 42
      ]
    where
