...
import           PlutusLedgerApi.V2          (BuiltinData, ScriptContext(scriptContextTxInfo),
                                             PubKeyHash, Datum(Datum), TxInInfo(txInInfoResolved),
                                             TxInfo, OutputDatum(OutputDatumHash, NoOutputDatum, OutputDatum),
                                             TxOut(txOutDatum, txOutValue), UnsafeFromData (unsafeFromBuiltinData))
import           PlutusLedgerApi.V2.Contexts (findDatum, getContinuingOutputs, txSignedBy, findOwnInput )
import           PlutusTx                    (compile, CompiledCode, unstableMakeIsData, FromData(fromBuiltinData),
                                             liftCode, unsafeApplyCode, makeLift)
import           PlutusTx.Prelude            ( Bool, Integer, Maybe(..), ($), (.), (&&), tail, isJust,
                                             traceError, traceIfFalse, Eq(..), take)
import           PlutusCore                  (Version (..))
import           Prelude                     (Show (show), span, IO)
import qualified Prelude                     ((/=) )
import           Data.String                 (IsString(fromString), String)
import           PlutusLedgerApi.V1.Value    (assetClassValueOf, AssetClass(AssetClass))
...
validator :: OracleParams -> CompiledCode (BuiltinData -> BuiltinData -> BuiltinData -> ())
validator oracle = $$(PlutusTx.compile [|| mkWrappedValidator ||])
  `PlutusTx.unsafeApplyCode` PlutusTx.liftCode (Version 1 0 0)  oracle
...
validatorCode = $$( PlutusTx.compile [|| mkWrappedValidatorLucid ||])
