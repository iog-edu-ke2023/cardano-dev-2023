...
import PlutusLedgerApi.V2         (TxOut(txOutAddress, txOutValue),
                                   ScriptContext(scriptContextTxInfo),
                                   TxInfo(txInfoReferenceInputs, txInfoMint),
                                   BuiltinData, adaToken,
                                   adaSymbol, TxInInfo(txInInfoResolved),
                                   txInfoInputs, txOutDatum, UnsafeFromData (unsafeFromBuiltinData),
                                   txOutValue,txInfoOutputs)
import PlutusLedgerApi.V1.Scripts  (ScriptHash (..))
import PlutusLedgerApi.V1.Value    (assetClassValueOf, AssetClass(AssetClass), valueOf)
import PlutusLedgerApi.V1.Address  (scriptHashAddress)
import PlutusLedgerApi.V2.Contexts (txSignedBy, ownCurrencySymbol)
import PlutusTx                    (compile, unstableMakeIsData,
                                   liftCode, makeLift, CompiledCode, unsafeApplyCode)
...
import           PlutusCore.Core   (Version (Version))
...
mkPolicy mp r ctx = case r of
    ...
    collateralOutput :: TxOut
    collateralOutput = case  collateralOutputs of
                        [o] -> o
                        _   -> traceError "expected exactly one collateral output"
        where
            collateralOutputs = [ o
                                | o <- txInfoOutputs info
                                , let oAddr = txOutAddress o
                                , oAddr == scriptHashAddress (mpCollateralValidator mp)
                                ]
...
policy :: MintParams -> CompiledCode (BuiltinData -> BuiltinData -> ())
policy np = $$(compile [|| mkWrappedPolicy ||])
    `unsafeApplyCode`
    liftCode (Version 1 0 0) np
