...
import           Plutus.Model            (Ada (Lovelace), DatumMode (..),
                                          ...
                                          userSpend, utxoAt,
                                          valueAt, mintValue, spendPubKey, scriptCurrencySymbol, datumAt,
                                          spendScript, signTx, refInputInline)
import           Plutus.Model.Validator   (TypedPolicy (..), TypedValidator (..))
import           Plutus.Model.Validator.V2 (mkTypedPolicy, mkTypedValidator)
...
import           Utilities                (validatorHash')
...
nftScript ref tn = mkTypedPolicy $ NFT.nftPolicy ref tn
...
oracleScript oracle = mkTypedValidator $ Oracle.validator oracle
...
collateralScript = mkTypedValidator Collateral.validator

-- Stablecoin policy
stableCoinScript :: Minting.MintParams -> TypedPolicy Minting.MintRedeemer
stableCoinScript mp = mkTypedPolicy $ Minting.policy mp
...
burnStablecoinTx us user policy ref dat burnVal = mconcat
                   ...
                   , payToKey user (adaValue 3_000_000)
...
testLiquidationCases = do
  ...
  let amountToMintU1 = 2
      ...
      collateral1 = adaValue 6_000_000
      collateral2 = adaValue 6_000_000
      collateral3 = adaValue 8_000_000
  [(ref,_)] <- utxoAt ov
...
testMintStableCoin = do
  ...
  sp <- spend u1 $ adaValue 3_000_000
